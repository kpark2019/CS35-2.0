...
def get_NYTIMES(url):
    # get the unfiltered data from the NY Times url
...
def get_BBC(url):
    # get the unfiltered data from the BBC url
    response = requests.get(url)
    if response.status_code == 404:  # page not found
        print("There was a problem with getting the page")
        print(url)
...
def get_most_used_words(url):
    # get the unfiltered data from the Wikipedia url
    response = requests.get(url)
    if response.status_code == 404:  # page not found
        print("There was a problem with getting the page")
        print(url)
...
def extract_words_Wiki(soup):
    AllDivs = soup.findAll('td')  # find all divs of the right class
    # print("AllDivs:", AllDivs)
    words_dict = []
    for div in AllDivs:
        # print(div.text.strip())
        headline = div.text.strip()
        #print(clean_word(headline))
        if not clean_word(headline) == "":
            words_dict.append(clean_word(headline))
        if len(words_dict) == 100:
            break
    words_dict.append("is")
    return words_dict

def extract_headlines_NY(soup):
    AllDivs = soup.findAll('h2', {'class': "headline"})  # find all divs of the right class
    # print("AllDivs:", AllDivs)
    words_dict = []
    for div in AllDivs:
        #print(div.text.strip())
        headline = div.text.strip().split(" ")
        #print(headline)
        for word in headline:
            word = clean_word(word)
            if not word == "":
                words_dict.append(word)
    wordCount = collections.Counter(words_dict)
    #print(wordCount)
    wikisoup = get_most_used_words("https://simple.wikipedia.org/wiki/Most_common_words_in_English")
    fillerWords = extract_words_Wiki(wikisoup)
...
def extract_headlines_BBC(soup):
    AllDivs = soup.findAll('span', {'class': "title-link__title-text"})  # find all divs of the right class
    # print("AllDivs:", AllDivs)
    words_dict = []
    for div in AllDivs:
        # print(div.text.strip())
...
def breaking_news():
    # We are going to use the web to figure out what the most pressing issue is in both the U.S. and the world
    # To do this, we will first scrape the US-page of the New York Times and the World Section from BBC
    # This will produce 'soup'
    nysoup = get_NYTIMES('https://www.nytimes.com/section/us?action=click&pgtype=Homepage&region=TopBar&module=HPMiniNav&contentCollection=U.S.&WT.nav=page')
    bbcsoup = get_BBC('http://www.bbc.com/news/world')
    # Then we will get the headlines from the soup
    nyhead = extract_headlines_NY(nysoup)
    bbchead = extract_headlines_BBC(bbcsoup)
    # Then we will compare the two dictionaries to see common words
    bothList = []
    for word in nyhead:
        if word in bbchead:
            bothList.append(word)
    #print(bothList)
    # Then we will search for the word in each respective dictionary and add up the total times it came up
    countList = {}
    for word in bothList:
        countList[word] = int(nyhead[word]) + int(bbchead[word])
    #print(countList)

    # Now we find which key word came up the most in both dictionaries
    maximum = 0
    maxKey = ""
    ...
    print(maxKey, "is the hottest topic in both international and national news.")
